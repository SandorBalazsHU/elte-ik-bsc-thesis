...
//SPECIAL MODIFY
template<TextureType type>
inline void TextureObject<type>::FromFileParallel(const std::string& filename) {
	...
	// Átalakítás 32bit RGBA formátumra, ha nem abban volt
	formattedSurf = SDL_ConvertSurfaceFormat(loaded_img, format, 0);
	if (formattedSurf == nullptr) {
		std::cout << "[AttachFromFile] Error converting image format: " << SDL_GetError() << std::endl;
		SDL_FreeSurface(loaded_img);
		return;
...
template<TextureType type>
inline void TextureObject<type>::bindTexture() {
	glGenTextures(1, &m_id);
